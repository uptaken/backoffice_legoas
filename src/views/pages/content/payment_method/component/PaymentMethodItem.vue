      ...
      <div class="row" :class="index > 0 ? 'mt-3' : ''" v-for="(data, index) in arr" :key="index">
        <div :class="is_view ? 'col-12' : 'col-10'">
          <div class="form-group">
            <label>Title</label>
            <input type="text" class="form-control" :value="'Step ' + (index + 1)" @keyup="(e) => title = e.target.value" :disabled="true">
          </div>
          <div class="form-group">
            <label>Description</label>
            <CustomEditor :id="index + '_reason_content'" :forceReload="!data.is_view" :readOnly="data.is_view" :value="data.is_view ? data.content : content" :contentType="data.is_view ? data.content_type : content_type" @onContentTypeChanged="(val) => content_type = val" @onDataChanged="(val) => content = val"/>
          </div>
        </div>
