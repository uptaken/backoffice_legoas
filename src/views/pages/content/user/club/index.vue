...
export default {
        ...
        return {
          base: null,
            ...
            fields: [
              ...
              { data: "category_sport_name", name: "category_sport.name", text: 'Category Sport', },
    ...
    async created() {
      this.base = new Base()
      base.save_current_page('Club', true)
    ...
    methods: {
      async get_data() {
        $('#please_wait_modal').modal('show')
          var url = base.url_api + '/user/club/all'
            ...
            if (response.status == "success") {
              ...
              for(var x in response_data){
                arr.push({
                  ...
                  'Category Sport' : response_data[x].name,
        ...
        async onActionClicked(row, action_button_index){
          ...
          if(action_button.id == "edit")
            window.location.href = '/club/action/edit/' + data.id
          else if (action_button.id === 'delete') {
            if (confirm('Are you sure want to delete this data?')) {
              $('#please_wait_modal').modal('show')
              var url = base.url_api + '/user/club/delete'
