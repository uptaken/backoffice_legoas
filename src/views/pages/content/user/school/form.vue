...
export default {
    ...
    async mounted() {
        ...
        if (this.$route.params.id != null) {
          base.save_current_page('Edit School')
            this.is_view_only = true
            this.get_data()
        }
        else
          base.save_current_page('Add School')
    },
    methods: {
        async get_data() {
            $('#please_wait_modal').modal('show')
            var url = base.url_api + '/user/school?id=' + this.$route.params.id
        ...
        async change_type() {
          if(confirm('Are you sure to change this school user to club?')){
            $('#please_wait_modal').modal('show')
        ...
        async action_btn(type) {
            var url = base.url_api + '/user/school'
            var data = this.form
            if (type === 'edit') {
      ...
      <div class="col-12">
        <div class="card">
          <div class="card-body">
              <div class="row">
                <div class="col-12">
                  <button class="btn btn-primary" v-on:click="change_type" v-if="this.$route.params.id != null">Change to Club</button>
                </div>
                ...
                <div class="col-12 mt-3">
                  <button class="btn btn-primary mr-1" v-on:click="action_btn('edit')">{{ is_view_only ? 'Edit' : 'Save' }} School</button>
                  <button class="btn btn-danger ml-1" v-on:click="action_btn('delete')" v-if="this.$route.params.id != null">Delete School</button>
