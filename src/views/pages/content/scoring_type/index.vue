...
export default {
            ...
            arr_top_button: [
              {
                ...
                text: 'Add Scoring Type',
                url: '/scoring-type/action/add',
              },
            ],
            arr_sort: [[0, "asc"]],
          force_reload: false,
    ...
    async created() {
      this.base = new Base()
      base.save_current_page('Scoring Type', true)
      ...
      async get_data() {
        ...
        async onActionClicked(row, action_button_index){
          ...
          if(action_button.id == "edit")
            window.location.href = '/scoring-type/action/edit/' + data.id
          else if (action_button.id === 'delete') {
            if (confirm('Are you sure want to delete this data?')) {
              $('#please_wait_modal').modal('show')
              var url = base.url_api + '/scoring-type/delete'
