...
export default {
    ...
    data() {
        return {
          base: null,
            ...
            title: "Category",
            fields: [
              { data: "id", name: "category.id", text: 'ID', },
              { data: "name", name: "category.name", text: 'Name', },
    ...
    async created() {
      this.base = new Base()
      base.save_current_page('Category', true)
    ...
    methods: {
      async onActionClicked(row, action_button_index){
        ...
        if(action_button.id == "edit")
          window.location.href = '/category/action/edit/' + data.id
        else if (action_button.id === 'delete') {
          if (confirm('Are you sure want to delete this data?')) {
            $('#please_wait_modal').modal('show')
            var url = base.url_api + '/category/delete'
