...
export default {
    ...
    data() {
        return {
          base: null,
            ...
            title: "Position Player",
            fields: [
              { data: "id", name: "position_player.id", text: 'ID', },
              { data: "name", name: "position_player.name", text: 'Name', },
            ...
            arr_top_button: [
              {
                ...
                text: 'Add Position Player',
                url: '/player-position/action/add',
              },
            ],
            arr_sort: [[0, "asc"]],
          force_reload: false,
    ...
    async created() {
      this.base = new Base()
      base.save_current_page('Position Player', true)
      ...
      async get_data() {
        ...
        async onActionClicked(row, action_button_index){
          ...
          if(action_button.id == "edit")
            window.location.href = '/player-position/action/edit/' + data.id
          else if (action_button.id === 'delete') {
            if (confirm('Are you sure want to delete this data?')) {
              $('#please_wait_modal').modal('show')
              var url = base.url_api + '/player-position/delete'
    ...
    <div class="card">
      <div class="card-body">
        <CustomTable
          ...
          :url="base.url_api + '/player-position'"/>
      </div>
    </div>
  </Layout>
</template>