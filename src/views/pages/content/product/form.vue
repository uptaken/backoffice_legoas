...
export default {
    ...
    async mounted() {
        ...
        if (this.$route.params.id != null) {
          base.save_current_page('Edit Venue')
            this.is_view_only = true
            this.get_data()
        }
        else
          base.save_current_page('Add Venue')
    },
    methods: {
        async get_data() {
            $('#please_wait_modal').modal('show')
            var url = base.url_api + '/venue?id=' + this.$route.params.id
                ...
                <div class="card">
                    <div class="card-body">
                        <div class="row">
                            ...
                            <div class="col-12">
                              <CustomEditor id="address" 
                                :readOnly="is_view_only" 
                                :value="form.address" 
                                :contentType="form.address_content_type" 
                                @onContentTypeChanged="(val) => $set(form, 'address_content_type', val)" 
                                @onDataChanged="(val) => $set(form, 'address', val)"/>
                            </div>
