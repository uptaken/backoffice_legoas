...
export default {
    ...
    async mounted() {
        ...
        if (this.$route.params.id != null) {
          base.save_current_page('Edit Player')
            this.is_view_only = true
            this.get_data()
        }
        else
          base.save_current_page('Add Player')
    ...
    methods: {
      ...
      async get_player_position() {
        ...
        var url = base.url_api + '/player-position/all?category_sport_id=' + this.form.registration_event.event_category_sport_category.event_category_sport.category_sport.id
      ...
      onCategorySportClicked(index){
        ...
        async get_data() {
            ...
            var url = base.url_api + '/event/registration/player?id=' + this.$route.params.id
            var response = await base.request(url);
        ...
        async action_btn(type) {
            var url = base.url_api + '/event/registration/player'
            var data = this.form
            var response = null
            if (type === 'edit') {
                if (this.is_view_only) {
                    ...
                    var selected_player_position = []
                    for(let player_position of this.arr_player_position){
                      if(player_position.id == this.form.player_position_id){
                        selected_player_position = player_position
                        break
                      }
                    }
                        data.is_publish = 1
                        data.player_position = selected_player_position
                        ...
                        <div class="row">
                          <div class="col-12">
                            <div class="form-group">
                              ...
                              <select class="form-control" v-model="form.player_position_id" :disabled="is_view_only">
                          ...
                          <div class="col-12">
                            <div class="form-group">
                              ...
                              <input v-model="form.back_number" type="text" class="form-control" placeholder="Back Number" :readonly="is_view_only" />
                            </div>
                          </div>
                          
                          <div class="col-12">
                            <div class="form-group">
                              <label>Student ID (NISN)</label>
                              <input v-model="form.nisn" type="text" class="form-control" placeholder="Student ID (NISN)" :readonly="is_view_only" />
                            </div>
                          </div>
