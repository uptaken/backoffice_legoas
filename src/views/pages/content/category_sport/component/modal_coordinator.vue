 ...
 */
export default {
  props: ['data', 'event_category_sport', 'arr', ],
  ...
  watch: {
    ...
    selected_coordinator(val){
      var data = {}
      for(let coordinator of this.arr_coordinator){
        if(val == coordinator.id){
          data = coordinator
          break
        }
      }
  ...
  methods: {
    async onCoordinatorSearch(search){
      var response = await base.request(base.url_api + "/coordinator?arr_not_exclude=" + JSON.stringify(this.arr) + "&search=" + search)
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="exampleModalLabel">{{ this.data != null ? 'Edit' : 'Add' }} Coordinator</h5>
          ...
          <div class="row">
            <div class="col-12">
              <div class="form-group">
                ...
                <select class="form-control" v-model="selected_coordinator">
                  <option value="" disabled selected>Choose Coordinator</option>
                  <option v-for="(coordinator, index) in arr_coordinator" :key="index" :value="coordinator.id">{{ coordinator.name }}</option>
                </select>
              ...
              <div class="form-group">
                ...
                <div>
                  <div class="form-check">
                    <input class="form-check-input" type="radio" disabled value="1" v-model="gender" id="radioGenderMale">
                    <label class="form-check-label" for="radioGenderMale">
                      Male
                    </label>
                  </div>
                  <div class="form-check">
                    <input class="form-check-input" type="radio" disabled value="0" v-model="gender" id="radioGenderFemale">
