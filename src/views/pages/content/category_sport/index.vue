...
export default {
    ...
    data() {
        return {
          base: null,
            ...
            fields: [
              { data: "id", text: 'ID', },
              { data: "name", name: "category_sport.name", text: 'Name', },
              { data: "created_at_format", name: "created_at_format", text: 'Created Date', },
              { data: "status_publish_format", name: "category_sport.status_publish", text: 'Status Publish', },
            ...
            arr_top_button: [
              {
                ...
                text: 'Add Category Sport',
                url: '/category-sport/action/add',
              },
            ],
            arr_sort: [[0, "asc"]],
          force_reload: false,
    ...
    async created() {
      this.base = new Base()
        ...
        base.save_current_page('Category Sport', true)
        // await this.get_data()
    },
    methods: {
        async get_data() {
        ...
        async onActionClicked(row, action_button_index){
          ...
          if(action_button.id == "edit")
            window.location.href = '/category-sport/action/edit/' + data.id
          else if (action_button.id === 'delete') {
            if (confirm('Are you sure want to delete this data?')) {
              $('#please_wait_modal').modal('show')
              var url = base.url_api + '/category-sport/delete'
                ...
                <div class="card">
                    <div class="card-body">
                        <CustomTable
                          ...
                          :url="base.url_api + '/category-sport'"/>
                    </div>
                </div>
            </div>
        </div>
