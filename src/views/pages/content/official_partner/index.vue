...
export default {
    ...
    data() {
        return {
          base: null,
            ...
            title: "Official Partner",
            fields: [
              { data: "id", name: "official_partner.id", text: 'ID', },
              { data: "url_image", name: "official_partner.file_name", text: 'Image', },
              { data: "created_at_format", name: "created_at_format", text: 'Created Date', },
              { data: "status_publish_format", name: "official_partner.status_publish", text: 'Status Publish', },
            ...
            arr_top_button: [
              {
                ...
                text: 'Add Official Partner',
                url: '/official-partner/action/add',
              },
            ],
            arr_sort: [[0, "asc"]],
          force_reload: false,
    ...
    async created() {
      this.base = new Base()
        base.save_current_page('Official Partner', true)
    ...
    methods: {
        ...
        async onActionClicked(row, action_button_index){
          ...
          if(action_button.id == "edit")
            window.location.href = '/official-partner/action/edit/' + data.id
          else if (action_button.id === 'delete') {
            if (confirm('Are you sure want to delete this data?')) {
              $('#please_wait_modal').modal('show')
              var url = base.url_api + '/official-partner/delete'
                ...
                <div class="card">
                    <div class="card-body">
                        <CustomTable
                          ...
                          :url="base.url_api + '/official-partner'"/>
                    </div>
                </div>
            </div>
        </div>
