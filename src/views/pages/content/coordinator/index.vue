...
export default {
    ...
    data() {
        return {
          base: null,
            ...
            title: "Coordinator",
            fields: [
              { data: "id", name: "coordinator.id", text: 'ID', },
              { data: "name", name: "coordinator.name", text: 'Name', },
              { data: "email", name: "coordinator.email", text: 'Email', },
              { data: "phone", name: "coordinator.phone", text: 'Phone', },
    ...
    async created() {
      this.base = new Base()
        base.save_current_page('Coordinator', true)
    ...
    methods: {
        ...
        async onActionClicked(row, action_button_index){
          ...
          if(action_button.id == "edit")
            window.location.href = '/coordinator/action/edit/' + data.id
          else if (action_button.id === 'delete') {
            if (confirm('Are you sure want to delete this data?')) {
              $('#please_wait_modal').modal('show')
              var url = base.url_api + '/coordinator/delete'
