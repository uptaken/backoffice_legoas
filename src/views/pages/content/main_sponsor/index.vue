...
export default {
    ...
    data() {
        return {
          base: null,
            ...
            title: "Main Sponsor",
            fields: [
              { data: "id", name: "main_sponsor.id", text: 'ID', },
              { data: "url_image", name: "main_sponsor.file_name", text: 'Image', },
              { data: "created_at_format", name: "created_at_format", text: 'Created Date', },
              { data: "status_publish_format", name: "main_sponsor.status_publish", text: 'Status Publish', },
            ...
            arr_top_button: [
              {
                ...
                text: 'Add Main Sponsor',
                url: '/main-sponsor/action/add',
              },
            ],
            arr_sort: [[0, "asc"]],
          force_reload: false,
    ...
    async created() {
      this.base = new Base()
      base.save_current_page('Main Sponsor', true)
    ...
    methods: {
        ...
        async onActionClicked(row, action_button_index){
          ...
          if(action_button.id == "edit")
            window.location.href = '/main-sponsor/action/edit/' + data.id
          else if (action_button.id === 'delete') {
            if (confirm('Are you sure want to delete this data?')) {
              $('#please_wait_modal').modal('show')
              var url = base.url_api + '/main-sponsor/delete'
